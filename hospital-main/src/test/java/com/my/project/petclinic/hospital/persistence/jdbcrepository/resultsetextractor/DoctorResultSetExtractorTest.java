...
class DoctorResultSetExtractorTest {

    @InjectMocks
    private DoctorResultSetExtractor subject;

    @Mock
    private ResultSet rs;

    @Test
    @DisplayName("should return doctor model when map resultSet")
    void shouldReturnDoctorWhenExtractDataTest() throws SQLException {
        ...
        //when
        final Doctor result = subject.extractData(rs);

        //then
        assertThat(result).isNotNull()
                .extracting("id", "name", "surName", "position")
                .contains(1L, "doctorName", "surName", "dentist");
        assertThat(result.getPatients()).isNotNull().hasSizeGreaterThan(0);
    }
